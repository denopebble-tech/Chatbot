import cv2, mediapipe as mp, time, numpy as numpy
...
def apply_filter(frame, type): 
    """Apply the specified filter to the frame."""
    if ftype == 'GREYSCALE': 
        return cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    elif ftype == 'SEPIA': 
        sepia_filter = np.array([[0.272, 0.534, 0.131], 0.349, 0.686, 0.189])
                ...
                for hand in results.multi_hand_landmarks: 
                    ...
                    tips = {name: (int(lm[idx].x * w), int(lm[idx].y * h))
                            for name, idx in {
                                ...
                                'ring': mp_hands.HandLandmark.PINKY_TIP
                            }.items()}
    ...
    display_img = cv2.cvtColor(filtered_img, cv2.COLOR_GRAY2BGR) if 
FILTERS[current_filter]=='GRAYSCALE' else filtered_img
